    ...
    <div className={`bg-white dark:bg-slate-900 border ${isCourt1 ? 'border-emerald-500/40' : 'border-sky-500/40'} rounded-3xl p-5 sm:p-6 shadow-xl space-y-4`}>
        ...
        <div className="flex items-center justify-between gap-2 p-2.5 rounded-2xl bg-slate-50 dark:bg-slate-950/70 border border-slate-200 dark:border-slate-800 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <button
              ...
              title="Shuffle the 4 active players into new teams"
            >
      ...
      <div className="rounded-2xl bg-[#064e3b] p-3 sm:p-5 border-2 border-emerald-400/80 shadow-xl">
        <div className="grid grid-cols-1 md:grid-cols-11 gap-3 items-stretch">
          ...
          {/* TEAM A (5 columns on desktop) */}
          <div className="md:col-span-5 flex flex-col justify-between bg-emerald-700/80 dark:bg-emerald-900/80 rounded-2xl p-3.5 border border-emerald-300/40 shadow-sm">
            ...
            {displayTeamA ? (
              <div className="space-y-2">
                {displayTeamA.map((player, idx) => (
                  <div
                    key={player.id}
                    className="bg-white dark:bg-slate-900 rounded-xl p-3 border border-emerald-400/40 shadow-sm flex items-center justify-between gap-2"
                  >
              ...
              <div className="py-6 text-center text-emerald-200/70 text-xs font-medium flex flex-col items-center justify-center gap-1.5">
                ...
                <span>Awaiting Players for Team A</span>
              </div>
            )}
          </div>

          {/* NET & KITCHEN (1 column on desktop) */}
          ...
          {/* TEAM B (5 columns on desktop - ALWAYS VISIBLE!) */}
          <div className="md:col-span-5 flex flex-col justify-between bg-emerald-700/80 dark:bg-emerald-900/80 rounded-2xl p-3.5 border border-sky-300/40 shadow-sm">
            <div className="flex items-center justify-between mb-3 border-b border-emerald-400/30 pb-2">
            ...
            {displayTeamB ? (
              <div className="space-y-2">
                {displayTeamB.map((player, idx) => (
                  <div
                    key={player.id}
                    className="bg-white dark:bg-slate-900 rounded-xl p-3 border border-sky-400/40 shadow-sm flex items-center justify-between gap-2"
                  >
                    <div className="min-w-0 flex-1 flex items-center gap-2">
              ...
              <div className="py-6 text-center text-emerald-200/70 text-xs font-medium flex flex-col items-center justify-center gap-1.5">
                <User className="w-6 h-6 opacity-60" />
                <span>Awaiting Players for Team B</span>
              </div>
            )}
          </div>
        </div>
